	...
	try {
		...
		const listEmployees = await EmployeeScanned.query()
			...
			.select(
				'estList.name',
				'estList.street',
				'estList.telephone_number',
				'estList.est_owner',
				'estList.image',
				'batch',
				'time_entered',
				'date_entered',
				'employee_scanned.account_id'
			)
		...
		const scannedIndiv = await EmployeeScanned.query()
			...
			.modify((list) => {
				...
				if (filterMonth) {
					list.whereRaw(
						'month(date_entered) = ?',
						monthValues.indexOf(filterMonth) + 1
					)
				}
...
router.get('/accounts/driver/passenger/:batch', async (req, res, next) => {
	try {
		...
		console.log(removeDuplicates(passengers))
