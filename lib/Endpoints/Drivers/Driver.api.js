...
router.post('/driver/create/passenger', async (req, res, next) => {
	...
	try {
		...
		const profile = await driver
			.$relatedQuery('UserProfile')
			.for(driver.account_id)

		const vehicle = await UserProfile.relatedQuery('DriverVehicles')
			.where('isActive', 1)
			.for(profile.user_id)
		if (vehicle === undefined) {
			const error = new Error('No Active Vehicle')
			throw error
		}

		const listOfCompanions = await Companions.query().where(
			'travel_id',
			travel_id
		)
		if (listOfCompanions === undefined) {
			const dummy = true
		}
		const getParent = await TravelHistory.query().findById(travel_id)

		const info = await UserProfile.relatedQuery('DriverScanned')
			.for(profile.user_id)
			.insertGraph({
				travel_id: travel_id,
				account_id: getParent.account_id,
				vehicle_used: vehicle[0].vehicle_id,
			})
		if (info) {
			for (const account of listOfCompanions) {
				await UserProfile.relatedQuery('DriverScanned')
					.for(profile.user_id)
					.insertGraph({
						travel_id: travel_id,
