...
router.post('/driver/create/passenger', async (req, res, next) => {
	...
	try {
		...
		const findVehicle = await DriverProfile.relatedQuery('DriverVehicles')
			.for(findProfile.driver_id)
			.where('isActive', 1)
		if (findVehicle[0] === undefined) {
			const error = new Error('Driver has no registered vehicle')
			throw error
		} else if (findVehicle.length === 0) {
			const error = new Error('Please select a vehicle to use')
			throw error
		}
		...
		const isOk = await DriverProfile.relatedQuery('DriverIdentitifier')
			...
			.insertGraph({
				...
				vehicle_used: findVehicle[0].vehicle_id,
			})
