...
router.post('/driver/create/passenger', async (req, res, next) => {
	...
	try {
		...
		const getTravel = await TravelHistory.query().findById(passengers.travel_id)
		const parentId = await UserProfile.query().findById(getTravel.user_id)
		...
		for (const id of getCompanions) {
			comId.push(id.users_id)
		}
		const getCompanionAcc = await Accounts.relatedQuery('UserProfile').for(
			comId
		)

		// if (!vehicle[0]) throw new Error('Driver has no vehicle')
		...
		if (insertParent) {
			for (const companion of getCompanionAcc) {
				console.log(companion.users_id)
