...
router.get('/list/establishment/entered/:user_id', async (req, res, next) => {
	const { user_id, pass_id } = req.params
	try {
		...
		const info = await UserProfile.query()
			.withGraphJoined('[EstEntered, EnteredInfo.NamesOfCompanions]')
			.findById(user_id)
		if (info === undefined) {
		...
		// let ids = []
		// for (const comps of info[0].CompanionsOfUser) {
		// 	ids.push(comps.users_id)
		// }

		// const companions = await UserProfile.query().findByIds(ids)
