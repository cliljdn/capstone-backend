...
<<<<<<< HEAD
				accountId.push(fillArray(ifExist.account_id, travelLog.email.length))
=======
...
<<<<<<< HEAD
		...
		for (let j = 0; j < accountId.length - 1; j++) {
			for (let a = 0; a < accountId[j].length; a++) {
				if (accountId[j][a] === companion[a].account_id) {
					accountId.push()
				}
				console.log(accountId[j][a])
				const insertToCompanion = await companion[a]
					.$relatedQuery('Companions')
					.insertGraphAndFetch({
						account_id: accountId[j][a],
						travel_id: fetchParentTravel.travel_id,
					})
			}
		}
