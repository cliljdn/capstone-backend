...
router.post('/accounts/create/travel', async (req, res, next) => {
	...
	try {
		...
		if (getTravelId) {
			for (let i = 0; i < arrCompanion.length; i++) {
				const getTry = await Accounts.relatedQuery('Companions')
					.for(arrCompanion[i].account_id)
					.insert({
				...
				const getData = await Accounts.relatedQuery('Companions')
					.for(findAcc.account_id)
					.insertAndFetch({
						travel_id: getTravelId.travel_id,
						parent_id: arrCompanion[i].account_id,
					})
					.catch((err) => {
						next(err)
					})
