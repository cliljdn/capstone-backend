...
router.get('/list/driver/passengers', async (req, res, next) => {
	try {
		...
		const passengers = await account[0].$relatedQuery('DriverScanned')
		let users = []
		for (const people of passengers) {
			const data = await UserProfile.query().findById(people.account_id)
			users.push(data)
		}
