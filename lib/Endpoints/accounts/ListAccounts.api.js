...
// list of profiles
router.get('/list/account/all/profiles', async (req, res, next) => {
	try {
		const auth = req.headers['authorization']
		if (!auth) {
			const error = new Error('No token provided')
		...
		const driverProfile = await DriverProfile.query()
		const employee = await EmployeeProfile.query()

		const allData = userProfile.concat(driverProfile).concat(employee)
...
router.get('/list/driver/passengers/:id', async (req, res, next) => {
	...
	try {
		...
		const txt = ['Passengers']
		const data = txt.concat(usersInfo)
		res.json(data)
	} catch (error) {
		next(error)
	}
